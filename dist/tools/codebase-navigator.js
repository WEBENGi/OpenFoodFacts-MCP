 ...
 */
export async function analyzeRepoStructure(repoPath, maxDepth = 2, excludePatterns = ["node_modules", ".git", "dist", "build"]) {
    ...
    async function analyzeDirectory(dirPath, depth) {
 ...
 */
export async function findImportantFiles(repoPath) {
 ...
 */
export async function readFileWithContext(repoPath, filePath) {
 ...
 */
export async function estimateRepoLanguages(repoPath) {
    const languages = {};
    // Helper function to count lines by language
    async function countLinesInDirectory(dirPath) {
        try {
            const entries = await readdir(dirPath);
            for (const entry of entries) {
                const entryPath = path.join(dirPath, entry);
 ...
 */
export async function searchInRepo(repoPath, pattern) {
    try {
 ...
 */
export async function getDependencyInfo(repoPath) {
 ...
 */
export async function generateCodebaseExplanation(repoPath, repoStructure, importantFiles, languageUsage, dependencies) {
    // Get content of the most important files for context
    const fileContents = {};
    // Prioritize README and other documentation files
    const docFiles = importantFiles.filter(file => ['README.md', 'readme.md', 'CONTRIBUTING.md', 'contributing.md'].includes(path.basename(file)));
