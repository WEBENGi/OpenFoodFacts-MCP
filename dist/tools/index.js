...
function wrapMcpServerTool(server) {
    ...
    server.tool = function (name, schemaOrCallback, handlerOrUndefined) {
        let schema, handler;
...
function convertZodSchemaToJsonSchema(schema) {
    ...
    if (schema && schema.shape) {
        for (const [propName, propSchema] of Object.entries(schema.shape)) {
            let property = {};
...
export function registerTools(server, developerMode = false) {
    ...
    server.server.setRequestHandler(CallToolRequestSchema, async (request, extra) => {
        ...
        if (name === "compareProducts") {
            try {
                const { nameOrBarcode1, nameOrBarcode2 } = args;
        ...
        if (name === "analyzeGitHubIssue") {
            try {
                const { org = "openfoodfacts", repo, issueNumber } = args;
        ...
        if (name === "analyzeGitHubIssuesByLabel") {
            try {
                const { org = "openfoodfacts", repo, label, state = "open", limit = 10 } = args;
                // Validate required arguments
                if (!repo || !repo.trim()) {
                    return {
                        isError: true,
        ...
        if (name === "createGitHubIssueRoadmap") {
            try {
                const { org = "openfoodfacts", repo, timeframe = "medium", focusArea } = args;
                // Validate required arguments
                if (!repo || !repo.trim()) {
                    return {
                        isError: true,
