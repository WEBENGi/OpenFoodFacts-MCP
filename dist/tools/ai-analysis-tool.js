import { z } from "zod";
import { requestSampling, createProductAnalysisRequest, createProductComparisonRequest, createRecipeSuggestionRequest } from "../sampling/sampling-service.js";
/**
 ...
 */
export function registerAIAnalysisTools(server) {
    // Register the product analysis tool
    server.tool("analyzeProduct", {
    ...
    server.tool("compareProducts", {
        ...
        try {
            ...
            const samplingRequest = createProductComparisonRequest(product1Data.product, product2Data.product);
            // Request LLM completion through the client
            const response = await requestSampling(server, samplingRequest);
            return {
                content: [
