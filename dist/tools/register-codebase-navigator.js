...
import { z } from "zod";
import { analyzeRepoStructure, findImportantFiles, estimateRepoLanguages, getDependencyInfo, generateCodebaseExplanation, searchInRepo, readFileWithContext, formatBytes, } from "./codebase-navigator.js";
/**
 ...
 */
export function registerCodebaseNavigatorTools(server) {
    // Register the tool to analyze repository structure
    server.tool("analyzeRepository", {
        ...
        try {
            ...
            const explanation = await generateCodebaseExplanation(localPath, repoStructure, importantFiles, languageUsage, dependencies);
    ...
    server.tool("explainCode", {
        ...
        try {
            ...
            const systemPrompt = "You are an expert software developer helping a newcomer understand code. " +
                ...
                if (includeDependencies) {
                    ...
                    if (Object.keys(dependencies).length > 0) {
                        ...
                        if (dependencies.python) {
                            dependenciesContent += "\n\n### Python Dependencies\n";
                            const pyDeps = dependencies.python.requirements
                                .map((req) => `- \`${req}\``)
                                .join('\n');
