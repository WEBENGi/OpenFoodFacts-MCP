 ...
 */
async function fetchGitHubIssues(repo: string, state: string = "open", labels: string = "") {
  try {
 ...
 */
function determineIssueType(issue: any): string {
  // Check labels first
  if (issue.labels && Array.isArray(issue.labels)) {
    const labelNames = issue.labels.map((label: any) => label.name.toLowerCase());
    
    if (labelNames.some((name: string) => name.includes('bug') || name.includes('error') || name.includes('fix'))) {
      return 'bug';
    }
    
    if (labelNames.some((name: string) => name.includes('enhancement') || name.includes('improve'))) {
      return 'enhancement';
    }
    
    if (labelNames.some((name: string) => name.includes('feature') || name.includes('new'))) {
      return 'feature';
    }
  }
 ...
 */
function determineComponentType(issue: any): string {
  // Check labels first
  if (issue.labels && Array.isArray(issue.labels)) {
    const labelNames = issue.labels.map((label: any) => label.name.toLowerCase());
    
    if (labelNames.some((name: string) => name.includes('frontend') || name.includes('ui') || name.includes('app') || name.includes('mobile'))) {
      return 'frontend';
    }
    
    if (labelNames.some((name: string) => name.includes('backend') || name.includes('server') || name.includes('api') || name.includes('database'))) {
      return 'backend';
    }
  }
 ...
 */
function createIssueAnalysisRequest(issue: any, orgRepoContext: string) {
  ...
  server.tool(
    ...
    async ({ repo, timeframe, focusArea }) => {
      try {
        ...
        const simplifiedIssues = limitedIssues.map((i: any) => ({
          number: i.number,
          title: i.title,
        ...
        const samplingRequest = createGitHubRoadmapRequest(
          simplifiedIssues,
          repo,
          timeframe,
          focusArea
